...
describe('BookingCalendar', () => {
  ...
  // Test that a time slot overlapping a booking starting before the search interval is not returned.
  it('does not return a time slot that overlaps a booking starting before the search interval', () => {
    const calendar = new BookingCalendar()
    const resource = new Resource('room-101', 'Study Room 101')
    ...
    const booking = new Booking(
      'booking-1',
      resource,
      new TimeSlot(new Date('2026-09-20T08:30:00'), new Date('2026-09-20T09:30:00'))
    )
    ...
    expect(availableTimeSlots[0].getStartTime()).toEqual(new Date('2026-09-20T09:30:00'))
  ...
  // Test that a booking ending exactly at the start of the requested date is not included.
  it('does not return a booking that ends exactly when the requested date starts', () => {
    const calendar = new BookingCalendar()
    const resource = new Resource('room-101', 'Study Room 101')
    ...
    const booking = new Booking(
      'booking-1',
      resource,
      new TimeSlot(new Date('2026-09-21T23:00:00'), new Date('2026-09-22T00:00:00'))
    )
  ...
  // Test that the getBookingById method trims whitespace from the booking id.
  it('trims whitespace when looking up a booking by id', () => {
    const calendar = new BookingCalendar()
    const resource = new Resource('room-101', 'Study Room 101')
    ...
    const foundBooking = calendar.getBookingById(' booking-1 ')
